...
#include "miivii_image_projection_based_fusion/pointpainting_fusion/node.hpp"

#include "autoware_point_types/types.hpp"

#include <miivii_image_projection_based_fusion/utils/geometry.hpp>
...
namespace miivii_image_projection_based_fusion
{
            ...
            "pointpainting_fusion", options)
  {
    ...
    RCLCPP_INFO(this->get_logger(), "create input in node!!!");
    ...
    // create detector
    detector_ptr_ = std::make_unique<miivii_image_projection_based_fusion::PointPaintingTRT>(
...
#pragma omp critical
          ...
          if (debugger_)
          {
            ...
            projected_point_2d[0] = static_cast<double>(normalized_projected_point.x() / p_z);
            projected_point_2d[1] = static_cast<double>(normalized_projected_point.y() / p_z);

            // std::cout << "normalized_projected_point" << normalized_projected_point << std::endl;

            debug_image_points.push_back(projected_point_2d);
          }
#endif
        }
      }
...
RCLCPP_COMPONENTS_REGISTER_NODE(miivii_image_projection_based_fusion::PointPaintingFusionNode)
