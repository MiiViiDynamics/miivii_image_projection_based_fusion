# miivii_image_projection_based_fusion

## Purpose

The `miivii_image_projection_based_fusion` is a package to fuse detected obstacles (bounding box or segmentation) from image and 3d pointcloud or obstacles (bounding box, cluster or segmentation).
...
```bash
ros2 launch miivii_image_projection_based_fusion pointpainting_fusion.launch.xml model_name:=pointpainting model_path:=/home/autoware/autoware_data/miivii_image_projection_based_fusion model_param_path:=$(ros2 pkg prefix miivii_image_projection_based_fusion --share)/config/pointpainting.param.yaml build_only:=true
```
